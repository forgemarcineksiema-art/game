#!/usr/bin/env python3
"""Create one original Blender-authored Tidebreak harbor water surface mesh.
...
OUTPUT = ROOT / "assets" / "models" / "blender_harbor_water_surface.gltf"


def main() -> int:
    _reset_scene()
    mesh = _build_water_surface_mesh()
    surface = bpy.data.objects.new("tb_blender_harbor_water_surface", mesh)
    ...
    print(f"Tidebreak Blender water surface exported: {OUTPUT}")
...
def _build_water_surface_mesh() -> bpy.types.Mesh:
    vertices: list[tuple[float, float, float]] = []
    faces: list[tuple[int, int, int]] = []

    _append_box(vertices, faces, center=(0.0, 0.006, 0.0), half_extents=(0.50, 0.006, 0.50))

    wave_ridges = [
        (-0.34, -0.18, 0.15, 0.004),
        (-0.08, 0.02, 0.19, 0.005),
        (0.22, 0.22, 0.17, 0.004),
    ]
    for x, z, length, height in wave_ridges:
        _append_box(vertices, faces, center=(x, 0.014 + height * 0.5, z), half_extents=(0.010, height * 0.5, length))

    cross_ripples = [
        (-0.24, 0.30, 0.10, 0.003),
        (0.18, -0.32, 0.12, 0.003),
    ]
    for x, z, width, height in cross_ripples:
        _append_box(vertices, faces, center=(x, 0.012 + height * 0.5, z), half_extents=(width, height * 0.5, 0.008))

    mesh = bpy.data.meshes.new("tb_blender_harbor_water_surface_mesh")
