...
- Post-v0.99: `docs\GAMEPLAY_REVIEW.md` remains the gameplay diagnosis gate. Jolt is the preferred production vehicle-runtime candidate; deterministic is the baseline/fallback. The route-pace gap is now narrower, so the next vehicle gap is Jolt-first controls/camera readability, especially reverse readability and camera reset/follow behavior.
