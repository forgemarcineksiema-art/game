...
2. The live opt-in Jolt vehicle switch now has comparable first-job live-loop evidence and camera-aware obstacle-proxy steering evidence. Deterministic reaches the service-run checkpoint in 139 frames through the runtime playthrough; Jolt reaches it in 213 frames through the same QA with no fallback or bounds hit. v0.56 shows both paths remain camera-readable in the obstacle proxy, but Jolt still makes much less forward progress (`x=11.97` versus deterministic `x=18.82`). Keep deterministic default, keep Jolt opt-in, and require Jolt tuning or collision-backed obstacle evidence before any default promotion.
