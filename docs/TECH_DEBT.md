...
1. Use `docs\GAMEPLAY_REVIEW.md` before selecting another autonomous milestone. v0.95 addressed the first physical-world-consequence gap by opening Low Dock Drain access after the Storm Pump Ticket, and v0.96 addressed the vehicle-feel evidence gap with Jolt-first steering/brake/reverse/route/camera metrics. The next high-value vehicle debt is deliberate Jolt live-driving tuning or promotion, not deterministic-only polish.
...
3. The Jolt vehicle switch now has comparable first-job live-loop evidence, camera-aware obstacle-proxy steering evidence, collision-backed obstacle replay telemetry, and v0.96 driving-feel metrics. Deterministic reaches the service-run checkpoint in 139 frames; Jolt reaches it in 212 frames with no fallback or bounds hit. v0.96 also records Jolt lateral deviation 0.399m, brake stop 0.233m, reverse distance 2.552m, steering yaw response 41.22 degrees, camera yaw lag 16.00 degrees, and zero obstacle overlap frames. `scripts\play.ps1` prefers Jolt only when the selected executable was built with Jolt; keep direct app/QA defaults deterministic until the next promotion milestone deliberately changes that.
