...
1. Use `docs\GAMEPLAY_REVIEW.md` and `docs\reviews\first-real-slice-direction-gate.md` before selecting another autonomous milestone. v0.95 addressed the first physical-world-consequence gap by opening Low Dock Drain access after the Storm Pump Ticket, v0.96 addressed the vehicle-feel evidence gap with Jolt-first steering/brake/reverse/route/camera metrics, v0.97 tightened vehicle camera follow, v0.98 proved Jolt route pace is not limited by the scripted throttle, v0.99 narrowed Jolt route pace from 212 to 169 frames with a conservative straight-drive assist, the post-v0.99 gates added explicit Jolt playthrough, authored dock-road edge clearance, broad-route collision response, extended recorded-input route evidence, sign-sensitive Jolt input semantics, and manual confirmation that current vehicle input/physics/camera feel is good in the Ferry Office/Jolt playtest path. The first Veyra world-foundation scaffold now exists as Cinder Harbor Reach with world-source compilation, capture proof, mesh/material terrain/road/shore presentation, and landmark/risk/route readability anchors. The next high-value debt is a compact Cinder Harbor crime/action route beat with risk, route choice, and recovery proof, not another vehicle proof, Ferry Office reaction, or art-only pass.
2. Use `python tools\capture_visual_smoke.py`, `python tools\capture_visual_smoke.py --scenario relay-to-service-log --report-json build\captures\capture_visual_smoke_midchain_report.json`, `python tools\capture_visual_smoke.py --scenario low-dock-drain-access --report-json build\captures\capture_visual_smoke_low_dock_report.json`, `python tools\playthrough_qa.py`, `python tools\physics_parity_qa.py`, opt-in `python tools\character_contact_qa.py`, opt-in `python tools\vehicle_physics_qa.py`, opt-in `python tools\vehicle_runtime_qa.py`, and opt-in `scripts\verify_jolt.ps1` as bounded visual/behavioral/physics evidence before asking for a manual pass. The playthrough QA now rejects reports missing runtime service-vehicle enter, dock-road checkpoint, and exit steps; the vehicle runtime QA rejects reports missing tap/brake/reverse/coast control checks, service-run route completion checks, collision-backed obstacle checks, deterministic/Jolt driving-feel checks, authored road-edge clearance checks, broad-route road-edge collision response checks, and extended recorded-input route checks.
3. The Jolt vehicle switch now has comparable first-job live-loop evidence, an explicit Jolt playthrough CTest, camera-aware obstacle-proxy steering evidence, collision-backed obstacle replay telemetry, v0.96 driving-feel metrics, v0.97 tighter camera follow, v0.98 route-pace sensitivity evidence, v0.99 controlled route-pace improvement, post-v0.99 authored road-edge clearance evidence, a broader straight/reverse/turn collision-response gate, a longer recorded-input route with several turns plus camera reset/readability evidence, sign-sensitive input semantics checks, and manual acceptance of current controls/physics/camera feel. Deterministic reaches the service-run checkpoint in 139 frames; Jolt reaches it in 169 frames with no fallback or bounds hit. v0.99 records Jolt route probes at 169, 163, and 158 frames for throttle 0.72, 0.86, and 1.0. The extended-route gate records Jolt at 360 frames, 16.634m route progress, 3 turns, 1.000m reverse, and 8.309 degrees post-reset camera yaw delta without hitting bounds. The current default-runtime decision remains deliberate: keep `scripts\play.ps1` on `preferred`, but keep no-flag `EngineApp` and QA defaults deterministic until broader world-collision/default-runtime evidence justifies changing default runtime semantics.
...
2. Deterministic vehicle movement remains the dependency-free direct-app and QA default after v0.99. Jolt VehicleConstraint/runtime now has feasibility, runtime-comparison, live-switch, controls-focused runtime QA, service-run route evidence, the same first-job enter-drive-exit-confirm playthrough evidence, an explicit Jolt playthrough CTest, camera-aware obstacle-proxy steering evidence, tuned obstacle proxy progress, collision-backed overlap replay evidence, required driving-feel metrics, tighter vehicle camera follow, route-pace sensitivity probes, a controlled straight-drive assist, authored road-edge clearance checks, and a preferred play-wrapper path when the Jolt executable is selected. It should still replace broader gameplay collision only through deliberate migration milestones, not as a side effect of QA hardening.
...
11. Veyra Cinder Harbor Reach is generated from world/area data. Terrain, roads, shoreline, landmarks, route anchors, and the risky cargo site now have first-pass mesh/material/readability replacements, but the path is still flat static placeholder mesh rendering, not terrain, road splines, textures, PBR, mesh collision, lighting, water simulation, traffic, AI, or final world art.
...
- The Jolt backend is validated through `windows-vs2022-debug-jolt` and opt-in `scripts\verify_jolt.ps1`, not through `scripts\verify.ps1`.
...
- Full no-flag default Jolt vehicle integration remains deferred until manual capture and broader dynamic/world contact prove the switched path is worth promoting. v0.37 proves opt-in live selection for the authored service vehicle, v0.37.1 fixes the first obvious tap/coast/reverse control bug, v0.48 proves the deterministic first-job runtime vehicle beat, v0.49 proves the same beat through opt-in Jolt, v0.59 closes the scripted obstacle-progress gap, v0.99 narrows the Jolt straight-route pace gap to 169 versus 139 deterministic, the first post-v0.99 gate proves the current route stays clear of two authored road-edge probes, the next gate proves a short broad-route road-edge collision response, and the current extended-route gate proves a longer recorded-input multi-turn route with reverse/camera reset/readability evidence. The conscious default-runtime decision is to keep Jolt preferred for playtest-facing Jolt-enabled builds, not to make every no-flag app/QA path Jolt by default. This is still not full gameplay collision, complete tuning, or a complete road model.
...
- v1.00 adds Veyra-only `assets/models/cinder_harbor_ground_patch.gltf`, `assets/models/cinder_harbor_road_plate.gltf`, and `assets/models/cinder_harbor_shore_shelf.gltf` through the fallback helper. They improve Cinder Harbor readability but are still untextured placeholder geometry and do not define collision.
