...
2. Deterministic vehicle movement remains the default live fallback after v0.65. Jolt VehicleConstraint now has feasibility, runtime-comparison, opt-in live-switch, controls-focused runtime QA, 240-frame service-run route evidence, the same first-job enter-drive-exit-confirm playthrough evidence, camera-aware obstacle-proxy steering evidence, tuned obstacle proxy progress, and collision-backed overlap replay evidence. It should still replace default gameplay only through a deliberate promotion milestone, not as a side effect of QA hardening.
