...
2. The live opt-in Jolt vehicle switch now has comparable first-job live-loop evidence and a first obstacle-proxy steering replay. Deterministic reaches the service-run checkpoint in 139 frames through the runtime playthrough; Jolt reaches it in 213 frames through the same QA with no fallback or bounds hit. v0.53 shows both paths can steer around a virtual obstacle proxy without bounds hits, but with very different offsets/progress. Keep deterministic default, keep Jolt opt-in, and require camera-aware obstacle/collision evidence or steering tuning before any default promotion.
...
2. Deterministic vehicle movement remains the default live fallback after v0.53. Jolt VehicleConstraint now has feasibility, runtime-comparison, opt-in live-switch, controls-focused runtime QA, 240-frame service-run route evidence, the same first-job enter-drive-exit-confirm playthrough evidence, and obstacle-proxy steering evidence, but it still should not replace default gameplay until camera-aware obstacle/collision replay evidence or steering tuning narrows the response gap.
