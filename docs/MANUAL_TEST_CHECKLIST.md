...
Optional renderer-owned visual capture:
...
Optional automated Ferry Office playthrough QA:

```powershell
python tools\playthrough_qa.py
```

This QA path is deterministic and off by default. It verifies the authored Service Call state chain and writes `build\playthroughs\ferry-office-service-call-report.json`; it does not prove camera feel, driving feel, route readability, or human input comfort.
...
- [ ] `scripts/play.ps1` launches the current Ferry Office Service Call in GDI playtest mode without needing long arguments.
- [ ] `scripts/play.ps1 -DebugUi`, `scripts/play.ps1 -MinimalUi`, `scripts/play.ps1 -Dx11 -Frames 120`, and `scripts/play.ps1 -FreeCursor` do what their names imply.
...
- [ ] `python tools\playthrough_qa.py` passes and writes `build\playthroughs\ferry-office-service-call-report.json` with `ferryOfficeJobComplete=true`.
...
- [ ] Scene report lists 9 colliders, 24 visual placeholders, 9 mesh assets, 40 mesh instances, 6 interactables, 1 traversal affordance, 1 vehicle, 6 route markers, and 5 objective markers.
- [ ] `python tools/scale_audit.py` reports no suspicious scale issues, or any issue is documented.
- [ ] `python tools/mesh_report.py` reports 9 mesh assets, 40 Ferry Office/service-yard/dock-road mesh instances, and 9 referenced `.gltf` model files.
...
- [ ] `assets/models/unit_box.gltf`, the v0.18 prop-kit meshes, `assets/models/ferry_notice_board.gltf`, `assets/models/blender_ferry_notice_board.gltf`, `assets/models/blender_cable_reel.gltf`, and `assets/models/blender_wet_road_surface.gltf` remain small original placeholder assets with scene-data license/provenance.
...
- [ ] The start camera is slightly off-axis rather than dead-center, while the manifest prompt remains immediately available.
