...
engine::Color DynamicSceneColor(std::string_view key, engine::Color authoredBaseColor, ScenePresentationState state)
{
    ...
    if (key == "ferry-office-drain-log-state") {
        return state.lowDockDrainLogged
            ? engine::Color {0.26f, 0.78f, 0.44f, 1.0f}
            : engine::Color {0.70f, 0.56f, 0.22f, 1.0f};
    }
...
bool IsKnownSceneColorKey(std::string_view key)
{
    return key == "dock-weathered-wood"
        ...
        || key == "cinder-brush-ground"
        || key == "oil-slick-asphalt"
        || key == "salt-cracked-concrete"
        || key == "black-rock-shore"
        || key == "low-tide-water"
        || key == "hazard-rust-red"
        || key == "beacon-amber-signal"
        || key == "cinder-mast-dark-metal"
        || key == "relay-oxidized-steel"
        || key == "cargo-warning-tarp"
        ...
        || key == "ferry-office-drain-log-state";
}
...
SceneMaterial SceneMaterialForKey(std::string_view key, ScenePresentationState state)
{
    ...
    if (key == "ferry-office-drain-log-state") {
        return PaintedMaterial(state.lowDockDrainLogged
            ? engine::Color {0.26f, 0.78f, 0.44f, 1.0f}
            : engine::Color {0.70f, 0.56f, 0.22f, 1.0f});
    }
